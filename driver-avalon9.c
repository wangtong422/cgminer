...
static inline uint32_t adjust_fan(struct avalon9_info *info, int id)
{
	...
	if (!tdiff && now_t < info->last_temp_time[id] + AVA9_DEFAULT_FAN_INTERVAL)
		goto out;
	...
	if (t >= info->temp_overheat[id]) {
		...
		if (info->fan_pct[id] < opt_avalon9_fan_max) {
			applog(LOG_WARNING, "Overheat detected on AV8-%d, increasing fan to max", id);
			info->fan_pct[id] = opt_avalon9_fan_max;
		}
	} else if (delta > 0) {
		...
		info->fan_pct[id] += opt_avalon9_fan_max; /* info->fan_pct[id] += delta + tdiff; */
	} else {
		/* Below target temperature */
		int diff = delta; /* int diff = tdiff; Sth. wrong with tdiff? It will keep lower than target */

		if (tdiff > 0) {
			int divisor = -delta / AVA9_DEFAULT_TEMP_HYSTERESIS + 1;
			...
			if (t < info->temp_target[id] - AVA9_DEFAULT_TEMP_HYSTERESIS && !tdiff)
				diff -= 1;
			diff = (delta == 0) ? 0 : diff;
		}
	...
	if (info->fan_pct[id] > opt_avalon9_fan_max)
		info->fan_pct[id] = opt_avalon9_fan_max;
	else if (info->fan_pct[id] < opt_avalon9_fan_min)
		info->fan_pct[id] = opt_avalon9_fan_min;
out:
	pwm = get_fan_pwm(info->fan_pct[id]);

	if (info->cutoff[id])
		pwm = get_fan_pwm(opt_avalon9_fan_max);
